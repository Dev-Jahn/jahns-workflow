...
- **D9 (2026-07-21 ruling, m1b-lease 보고 ④1·④2·⑤).** ⑴ M1-B acceptance의 lease principal은
  promoted-contracts 문언의 triple(`owner_token + fencing_epoch + entity_version`)로 충족한다.
  ADR-0013 full principal의 project/executor 결속 축은 store v2 migration으로 분리한다
  (`fix/lease-principal-project-executor-binding` — leases `(entity_kind, entity_id)` UNIQUE/FK
  승격 포함). ⑵ `feat/run-effect-protocol`은 store에 **additive 확장**(package-internal
  shared-transaction/guarded-transition surface, 필요 시 migration registry 경유 v-next)을
  허용한다 — 기존 v1 정의·기존 테스트 재작성은 여전히 금지.
- **D10 (2026-07-21 ruling, m1b-lease 보고 ④3).** exit fixture 8은 공동 소유로 판정한다:
  stale-principal 거부 절은 feat/run-lease-fencing(완료), 같은 action의 effect-kind별 semantic
  idempotency·dedupe 절은 feat/run-effect-protocol. gate/m1b-exit는 두 절을 각 소유 테스트로
  확인한다.
...
| ADR-0013 "DB·artifact permission과 symlink" 절 (mode 고정·WAL/SHM owner-mode·no-follow) | fix/run-store-permission-hardening (2026-07-21 추가 — m1b-store 보고 ④11이 확인한 귀속 공백) |
...
| fix/run-store-permission-hardening | store-kernel | ADR-0013 permission/no-follow 절 (2026-07-21 추가) | test_run_store.py 확장 |
...
C: run-effect-protocol ∥ run-verification-preflight ∥ run-store-permission-hardening
