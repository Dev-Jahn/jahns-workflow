...
`취소, quiescence, cleanup 안전 계약`에 정의된 독립 취소·quiescence 안전 계약, 그리고
`docs/adr/ADR-0013-operational-threat-model.md`의 lease·lock principal 계약을 test layer에 결속한다.
현재 실물 test는 전부
...
| I-07 | `MigrationSunsetTests.test_pre_0_9_layout_is_refused_with_0_11_x_guidance`<br>`MigrationSunsetTests.test_plain_machine_state_is_refused_without_registry_merge`<br>`MigrationSunsetTests.test_completed_0_11_seed_and_empty_scaffolding_are_accepted` | TODO(M1) | `MigrationSunsetTests.test_pending_worktree_marker_is_refused_without_repair` | partial — 현 sunset의 unsupported source no-write·completed seed no-op·pending marker no-repair만 coverage. 새 adoption의 positive two-run idempotence·atomic-replace crash resume·file-move recovery·symlink refusal·previewable 동작은 gap/TODO(M1) |
...
| ADR-0013 `Lease·lock principal` | TODO(M1-B) — 현행 test 없음 | TODO(M1-B) | TODO(M1-B): matching epoch/version에서도 다른 `owner_token`의 CAS 거부<br>OS lock 획득 뒤 DB token·epoch·version 재확인<br>reclaim과 구 owner mutation의 race | gap — 세 fault 모두 M1-B 소유; typed refusal·no mutation과 stale owner fencing을 새 kernel에서 검증해야 함 |
