...
- Stay strictly within scope. Modify only files inside this worktree. A problem outside the task's
  scope goes in the WAYSTONE_REPORT `escalations` list, not fixed silently.
- The environment is already prepared. Do NOT install dependencies, use network tools or
  network-capable shell commands, or work around a missing environment — use what is provided. If
  something you need is absent, escalate it.
- If blocked, do NOT revert, reset, or clean away completed work. Preserve the worktree as-is and
  report the blocker under WAYSTONE_REPORT `limitations`, plus any resulting uncertainty under `risks`.
