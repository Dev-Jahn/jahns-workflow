---
name: audit
description: This skill should be used when the user runs "/jahns-workflow:audit", asks to "audit the SSOT", "verify the design doc", "check the spec for errors", or after a bulk SSOT edit triggers the quarantine rule. Runs a scoped, tiered independent verification of the SSOT — it never runs production test suites or GPU builds.
...
scale to long projects or expensive targets (GPU kernels, hour-long suites). So: audit only
...
- **T3 — oracle execution (ONLY sections listed under `oracles:` in config).** Each oracle entry declares a section anchor and a cheap CPU-scale reference check (seconds, not minutes; fp64 reference vs sequential oracle is the canonical pattern). Agents may write a small throwaway reference implementation from the spec text and compare against the declared oracle. **Never** run the production test suite, compile GPU kernels, or anything resembling them. If no oracles are declared, skip T3 and note which in-scope sections *would* benefit from one.
